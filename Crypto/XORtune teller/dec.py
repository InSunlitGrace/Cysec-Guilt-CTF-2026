...
# Recover initial state S0 from the first 4-byte block
S0 = int.from_bytes(ct[:4], "big") ^ int.from_bytes(known_prefix[:4], "big")
print(f"Recovered seed: {hex(S0)}")
...
# Verify with the "completely unrelated integers"
ints = [130, 101, 250, 179, 100]
rng = XORshift(S0)

# Advance RNG past all ciphertext blocks
for _ in range(len(ct) // 4):
    rng.next()

print("\nVerification (should all be True):")
